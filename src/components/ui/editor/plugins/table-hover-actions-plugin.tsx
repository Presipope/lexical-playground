  ...
  const debouncedOnMouseMove = useDebounce(
    (event: MouseEvent) => {
      ...
      if (tableDOMElement) {
        ...
        if (
          parentElement &&
          parentElement.classList.contains('tableScrollableWrapper')
        ) {
          tableHasScroll =
...
function getMouseInfo(event: MouseEvent): {
  ...
  if (isHTMLElement(target)) {
    const tableDOMNode = target.closest<HTMLElement>(
      'td.editor-table-cell, th.editor-table-cell'
    )

    const isOutside = !(
      tableDOMNode ||
      target.closest<HTMLElement>('button.table-add-rows') ||
      target.closest<HTMLElement>('button.table-add-columns') ||
      target.closest<HTMLElement>('div.table-cell-resizer')
    )
